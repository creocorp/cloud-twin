...
def make_router(service: GcpCloudFunctionsService) -> APIRouter:
    ...
    async def create_function(project: str, location: str, request: Request) -> JSONResponse:
        try:
            body = await request.json()
        except Exception:
            body = {}
        name = body.get("name", "").split("/")[-1] or request.query_params.get("functionId", "")
        runtime = body.get("buildConfig", {}).get("runtime", "python311")
        entrypoint = body.get("buildConfig", {}).get("entryPoint", "")
        try:
            fn = await service.create_function(project, location, name, runtime, entrypoint)
            return JSONResponse({"name": fn.full_name, "runtime": fn.runtime}, status_code=200)
    ...
    async def list_functions(project: str, location: str) -> JSONResponse:
        ...
        return JSONResponse({"functions": [{"name": f.full_name, "runtime": f.runtime} for f in fns]})

    @router.get("/v2/projects/{project}/locations/{location}/functions/{function_name}")
    async def get_function(project: str, location: str, function_name: str) -> JSONResponse:
        try:
    ...
    @router.delete("/v2/projects/{project}/locations/{location}/functions/{function_name}")
    async def delete_function(project: str, location: str, function_name: str) -> Response:
        try:
    ...
    @router.post("/v2/projects/{project}/locations/{location}/functions/{function_name}:call")
    async def invoke(project: str, location: str, function_name: str, request: Request) -> JSONResponse:
        try:
