...
use super::models::{SnsMessage, SnsSubscription, SnsTopic};
...
impl SnsService {
    pub fn new(db: Database) -> Self { SnsService { db } }

    fn topic_arn(name: &str) -> String {
        format!("arn:aws:sns:{REGION}:{ACCOUNT_ID}:{name}")
    }
    fn sub_arn(name: &str) -> String {
        format!("arn:aws:sns:{REGION}:{ACCOUNT_ID}:{name}:{}", Uuid::new_v4().simple())
    }
    ...
    pub async fn list_topics(&self) -> Result<Vec<SnsTopic>> {
        self.db.conn.call(|conn| {
            let mut stmt = conn.prepare(
                "SELECT arn, name, created_at FROM sns_topics ORDER BY name"
            )?;
            let rows = stmt.query_map([], |r| Ok(SnsTopic {
                arn:        r.get(0)?,
                name:       r.get(1)?,
                created_at: r.get(2)?,
    ...
    pub async fn delete_topic(&self, arn: &str) -> Result<()> {
        let a = arn.to_string();
        self.db.conn.call(move |conn| {
            conn.execute("DELETE FROM sns_topics WHERE arn = ?1", rusqlite::params![a])?;
            Ok(())
        }).await.map_err(|e| anyhow::anyhow!("{e}"))
    }

    pub async fn subscribe(&self, topic_arn: &str, protocol: &str, endpoint: &str) -> Result<String> {
        // Validate topic exists
        let ta = topic_arn.to_string();
        let exists: bool = self.db.conn.call(move |conn| {
            let n: i64 = conn.query_row(
                "SELECT COUNT(*) FROM sns_topics WHERE arn = ?1",
                rusqlite::params![ta], |r| r.get(0),
            )?;
            Ok(n > 0)
        }).await.map_err(|e| anyhow::anyhow!("{e}"))?;
        if !exists { bail!("NotFound: topic {topic_arn}"); }

        // Derive sub name from endpoint
        let sub_name = topic_arn.rsplit(':').next().unwrap_or("topic");
        let sub_arn  = format!("arn:aws:sns:{REGION}:{ACCOUNT_ID}:{sub_name}:{}", Uuid::new_v4().simple());
    ...
    pub async fn list_subscriptions_by_topic(&self, topic_arn: &str) -> Result<Vec<SnsSubscription>> {
        let ta = topic_arn.to_string();
        self.db.conn.call(move |conn| {
            let mut stmt = conn.prepare(
            ...
            let rows = stmt.query_map(rusqlite::params![ta], |r| Ok(SnsSubscription {
                arn:        r.get(0)?,
                topic_arn:  r.get(1)?,
                protocol:   r.get(2)?,
                endpoint:   r.get(3)?,
    ...
    pub async fn publish(&self, topic_arn: &str, message: &str, subject: Option<&str>) -> Result<String> {
