...
pub async fn handle_json(state: &Arc<AppState>, target: &str, body: serde_json::Value) -> Response {
    match target {
        "AmazonSQS.CreateQueue" => {
            ...
            if name.is_empty() {
                return json_error_response(StatusCode::BAD_REQUEST, "MissingParameter", "QueueName required");
            }
            match svc(state).create_queue(name).await {
                Ok(url) => (StatusCode::OK, axum::Json(serde_json::json!({ "QueueUrl": url }))).into_response(),
                Err(e)  => json_error_response(StatusCode::INTERNAL_SERVER_ERROR, "InternalError", &e.to_string()),
            }
        }
        "AmazonSQS.ListQueues" => {
            let prefix = body.get("QueueNamePrefix").and_then(|v| v.as_str()).unwrap_or("");
            match svc(state).list_queues(prefix).await {
                Ok(urls) => (StatusCode::OK, axum::Json(serde_json::json!({ "QueueUrls": urls }))).into_response(),
                Err(e)   => json_error_response(StatusCode::INTERNAL_SERVER_ERROR, "InternalError", &e.to_string()),
            }
        }
        "AmazonSQS.GetQueueUrl" => {
            ...
            match svc(state).get_queue_url(name).await {
                Ok(url) => (StatusCode::OK, axum::Json(serde_json::json!({ "QueueUrl": url }))).into_response(),
                Err(e) if e.to_string().contains("QueueDoesNotExist") =>
                    json_error_response(StatusCode::BAD_REQUEST, "QueueDoesNotExist", &e.to_string()),
                Err(e) => json_error_response(StatusCode::INTERNAL_SERVER_ERROR, "InternalError", &e.to_string()),
            }
        }
        "AmazonSQS.SendMessage" => {
            let url  = body.get("QueueUrl").and_then(|v| v.as_str()).unwrap_or("");
            let body_str = body.get("MessageBody").and_then(|v| v.as_str()).unwrap_or("");
            match svc(state).send_message(url, body_str).await {
                Ok((mid, _)) => (StatusCode::OK, axum::Json(serde_json::json!({
                    "MessageId":        mid,
        ...
        "AmazonSQS.ReceiveMessage" => {
            let url = body.get("QueueUrl").and_then(|v| v.as_str()).unwrap_or("");
            let max = body.get("MaxNumberOfMessages").and_then(|v| v.as_i64()).unwrap_or(1);
            match svc(state).receive_messages(url, max).await {
                Ok(msgs) => {
                    ...
                    (StatusCode::OK, axum::Json(serde_json::json!({ "Messages": items }))).into_response()
                }
                Err(e) if e.to_string().contains("QueueDoesNotExist") =>
                    json_error_response(StatusCode::BAD_REQUEST, "QueueDoesNotExist", &e.to_string()),
                Err(e) => json_error_response(StatusCode::INTERNAL_SERVER_ERROR, "InternalError", &e.to_string()),
        ...
        "AmazonSQS.DeleteMessage" => {
            let rh = body.get("ReceiptHandle").and_then(|v| v.as_str()).unwrap_or("");
        ...
        "AmazonSQS.ChangeMessageVisibility" => {
            let url = body.get("QueueUrl").and_then(|v| v.as_str()).unwrap_or("");
            let rh  = body.get("ReceiptHandle").and_then(|v| v.as_str()).unwrap_or("");
            let to  = body.get("VisibilityTimeout").and_then(|v| v.as_i64()).unwrap_or(30);
        ...
        _ => json_error_response(StatusCode::BAD_REQUEST, "InvalidAction", &format!("Unknown SQS target: {target}")),
    }
}
