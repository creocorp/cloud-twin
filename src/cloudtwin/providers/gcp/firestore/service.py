...
class FirestoreService:
    ...
    async def set_document(self, project: str, collection: str, document_id: str, fields: dict) -> FirestoreDocument:
        ...
        await self._telemetry.emit("gcp", "firestore", "set_document", {"project": project, "collection": collection})
        return saved

    async def get_document(self, project: str, collection: str, document_id: str) -> FirestoreDocument:
        doc = await self._repo.get(project, collection, document_id)
        if not doc:
            raise NotFoundError(f"Document not found: {collection}/{document_id}")
        return doc

    async def delete_document(self, project: str, collection: str, document_id: str) -> None:
        await self._repo.delete(project, collection, document_id)
        await self._telemetry.emit("gcp", "firestore", "delete_document", {"project": project, "collection": collection})

    async def list_documents(self, project: str, collection: str) -> list[FirestoreDocument]:
