...
impl BlobService {
    pub fn new(db: Database, account: String) -> Self {
        BlobService { db, account }
    }
    ...
    pub async fn list_containers(&self) -> Result<Vec<AzureContainer>> {
        ...
        self.db
            ...
            .call(move |conn| {
                ...
                let rows = stmt
                    .query_map(rusqlite::params![acc], |r| {
                        Ok(AzureContainer {
                            name: r.get(0)?,
                            account: r.get(1)?,
                            created_at: r.get(2)?,
    ...
    pub async fn get_blob(&self, container: &str, blob_name: &str) -> Result<AzureBlob> {
        ...
        let bn = blob_name.to_string();
        let acc = self.account.clone();
        let blob: Option<AzureBlob> = self
            .db
            .conn
            .call(move |conn| {
                Ok(conn.query_row(
    ...
    pub async fn list_blobs(
        ...
        self.db
            ...
            .call(move |conn| {
                ...
                let rows = stmt
                    .query_map(rusqlite::params![acc, c, pref], |r| {
                        Ok(AzureBlob {
                            container: r.get(0)?,
                            account: r.get(1)?,
                            name: r.get(2)?,
