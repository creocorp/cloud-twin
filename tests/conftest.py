...
def _test_config(**overrides) -> Config:
    ...
    ses_config = overrides.pop("ses_config", SesConfig(strict_verification=False, smtp=SmtpConfig()))
    return Config(
...
async def strict_client():
    ...
    app = create_app(_test_config(ses_config=SesConfig(strict_verification=True, smtp=SmtpConfig())))
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        async with app.router.lifespan_context(app):
            yield c
