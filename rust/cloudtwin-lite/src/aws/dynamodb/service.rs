...
impl DynamoDBService {
    ...
    pub async fn create_table(
        ...
        self.get_table(name)
            .await?
            .ok_or_else(|| anyhow::anyhow!("create_table: not found after insert"))
    }
    ...
    pub async fn delete_table(&self, name: &str) -> Result<()> {
        ...
        self.db
            ...
            .call(move |conn| {
                conn.execute(
                    "DELETE FROM dynamo_items WHERE table_name = ?1",
                    rusqlite::params![n],
                )?;
                conn.execute(
                    "DELETE FROM dynamo_tables WHERE name = ?1",
                    rusqlite::params![n2],
                )?;
                Ok(())
            })
    ...
    pub fn table_to_json(&self, t: &DynamoTable) -> serde_json::Value {
        ...
        let ad: serde_json::Value =
            serde_json::from_str(&t.attribute_definitions).unwrap_or_default();
        serde_json::json!({
            "TableName":              t.name,
    ...
    pub async fn put_item(&self, table_name: &str, item: serde_json::Value) -> Result<()> {
        let table = self.get_table(table_name).await?.ok_or_else(|| {
            anyhow::anyhow!("ResourceNotFoundException: table {table_name} not found")
        })?;
    ...
    pub async fn get_item(
        ...
        let tn = table_name.to_string();
        let item_str: Option<String> = self
            .db
            .conn
            .call(move |conn| {
                Ok(conn
    ...
    pub async fn scan(
        ...
        let rows: Vec<String> = self
            ...
            .call(move |conn| {
                ...
                let rows = stmt
                    .query_map(rusqlite::params![tn, lim], |r| r.get::<_, String>(0))?
                    .collect::<rusqlite::Result<Vec<_>>>()?;
                Ok(rows)
            })
            .await
