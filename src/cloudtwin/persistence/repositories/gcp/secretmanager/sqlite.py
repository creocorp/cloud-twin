...
class SqliteGcpSecretRepository(GcpSecretRepository):
    ...
    def _row(self, row) -> GcpSecret:
        return GcpSecret(id=row["id"], project=row["project"], name=row["name"],
    ...
    async def save(self, secret: GcpSecret) -> GcpSecret:
        await self._db.conn.execute(
            ...
            (secret.project, secret.name, secret.full_name, secret.created_at or _now()),
        )
        await self._db.conn.commit()
        return await self.get(secret.full_name)

    async def delete(self, full_name: str) -> None:
        await self._db.conn.execute("DELETE FROM gcp_secrets WHERE full_name = ?", (full_name,))
        await self._db.conn.commit()
...
class SqliteGcpSecretVersionRepository(GcpSecretVersionRepository):
    ...
    def _row(self, row) -> GcpSecretVersion:
        return GcpSecretVersion(
            id=row["id"], secret_full_name=row["secret_full_name"],
            version_id=row["version_id"], payload=row["payload"],
    ...
    async def save(self, version: GcpSecretVersion) -> GcpSecretVersion:
        await self._db.conn.execute(
            ...
            (version.secret_full_name, version.version_id, version.payload, version.state, version.created_at or _now()),
        )
        await self._db.conn.commit()
        return version
    ...
    async def delete_all(self, secret_full_name: str) -> None:
        await self._db.conn.execute(
            "DELETE FROM gcp_secret_versions WHERE secret_full_name = ?", (secret_full_name,)
        )
        await self._db.conn.commit()
