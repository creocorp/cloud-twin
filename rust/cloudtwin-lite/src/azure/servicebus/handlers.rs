...
pub fn router() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route("/:namespace/queues/:queue",                       put(create_queue).delete(delete_queue))
        .route("/:namespace/queues/:queue/messages",              post(send_message).get(receive_messages))
        .route("/:namespace/queues/:queue/messages/:lock_token",  delete(complete_message))
        .route("/:namespace/queues/:queue/messages/:lock_token/abandon", post(abandon_message))
        // Topics
        .route("/:namespace/topics",                              get(list_topics))
        .route("/:namespace/topics/:topic",                       put(create_topic).delete(delete_topic))
        .route("/:namespace/topics/:topic/messages",              post(send_topic_message))
        // Subscriptions
        .route("/:namespace/topics/:topic/subscriptions",                         get(list_subscriptions))
        .route("/:namespace/topics/:topic/subscriptions/:sub",                    put(create_subscription))
        .route("/:namespace/topics/:topic/subscriptions/:sub/messages",           get(receive_sub_messages))
        .route("/:namespace/topics/:topic/subscriptions/:sub/messages/:lock_token", delete(complete_sub_message))
}
...
async fn create_queue(
    ...
    match svc(&state).create_queue(&queue).await {
        Ok(q)  => (StatusCode::CREATED, Json(serde_json::json!({ "name": q.name }))).into_response(),
...
async fn list_queues(State(state): State<Arc<AppState>>, Path(_ns): Path<String>) -> Response {
    match svc(&state).list_queues().await {
        Ok(qs) => {
            let items: Vec<_> = qs.iter().map(|q| serde_json::json!({ "name": q.name })).collect();
...
async fn send_message(
    ...
    match svc(&state).send_message(&queue, &body_str).await {
        Ok(mid) => (StatusCode::CREATED, Json(serde_json::json!({ "messageId": mid }))).into_response(),
        Err(e)  => err(StatusCode::INTERNAL_SERVER_ERROR, &e.to_string()),
    }
}
...
async fn receive_messages(
    ...
    match svc(&state).receive_messages(&queue, 1).await {
        Ok(msgs) => {
            let items: Vec<_> = msgs.iter().map(|m| serde_json::json!({
...
async fn create_topic(
    ...
    match svc(&state).create_topic(&topic).await {
        Ok(t)  => (StatusCode::CREATED, Json(serde_json::json!({ "name": t.name }))).into_response(),
        Err(e) => err(StatusCode::INTERNAL_SERVER_ERROR, &e.to_string()),
    }
}
...
async fn list_topics(State(state): State<Arc<AppState>>, Path(_ns): Path<String>) -> Response {
    match svc(&state).list_topics().await {
        Ok(ts) => {
            let items: Vec<_> = ts.iter().map(|t| serde_json::json!({ "name": t.name })).collect();
            (StatusCode::OK, Json(serde_json::json!({ "value": items }))).into_response()
        }
        Err(e) => err(StatusCode::INTERNAL_SERVER_ERROR, &e.to_string()),
    }
...
async fn create_subscription(
    ...
    match svc(&state).create_subscription(&topic, &sub).await {
        Ok(s)  => (StatusCode::CREATED, Json(serde_json::json!({ "name": s.name, "topic": s.topic }))).into_response(),
        Err(e) => err(StatusCode::INTERNAL_SERVER_ERROR, &e.to_string()),
    }
}
...
async fn list_subscriptions(
    ...
    match svc(&state).list_subscriptions(&topic).await {
        Ok(subs) => {
            let items: Vec<_> = subs.iter().map(|s| serde_json::json!({ "name": s.name })).collect();
            (StatusCode::OK, Json(serde_json::json!({ "value": items }))).into_response()
        }
        Err(e) => err(StatusCode::INTERNAL_SERVER_ERROR, &e.to_string()),
    }
...
async fn receive_sub_messages(
    ...
    match svc(&state).receive_subscription_messages(&topic, &sub, 1).await {
        Ok(msgs) => {
            let items: Vec<_> = msgs.iter().map(|m| serde_json::json!({
                "messageId": m.message_id,
                "body":      m.body,
