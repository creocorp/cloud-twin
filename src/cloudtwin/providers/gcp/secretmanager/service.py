...
class GcpSecretManagerService:
    ...
    async def create_secret(self, project: str, name: str) -> GcpSecret:
        ...
        if existing:
            return existing
        secret = GcpSecret(project=project, name=name, full_name=full_name, created_at=_now())
        saved = await self._secrets.save(secret)
        await self._telemetry.emit("gcp", "secretmanager", "create_secret", {"project": project, "name": name})
        return saved

    async def add_secret_version(self, project: str, name: str, payload: bytes) -> GcpSecretVersion:
        full_name = _full_name(project, name)
        ...
        await self._telemetry.emit("gcp", "secretmanager", "add_secret_version", {"name": name})
        return saved

    async def access_secret_version(self, project: str, name: str, version_id: str = "latest") -> GcpSecretVersion:
        full_name = _full_name(project, name)
    ...
    async def delete_secret(self, project: str, name: str) -> None:
        ...
        await self._telemetry.emit("gcp", "secretmanager", "delete_secret", {"name": name})
