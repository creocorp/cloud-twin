...
impl StorageService {
    pub fn new(db: Database, project: String) -> Self { StorageService { db, project } }
    ...
    pub async fn create_bucket(&self, name: &str, location: Option<&str>) -> Result<GcsBucket> {
        ...
        Ok(GcsBucket { name: n2, project: p2, location: loc2, created_at: n3 })
    }
    ...
    pub async fn list_buckets(&self) -> Result<Vec<GcsBucket>> {
        ...
        self.db.conn.call(move |conn| {
            ...
            let rows = stmt.query_map(rusqlite::params![p], |r| Ok(GcsBucket {
                name:       r.get(0)?,
                project:    r.get(1)?,
                location:   r.get(2)?,
                created_at: r.get(3)?,
    ...
    pub async fn delete_bucket(&self, name: &str) -> Result<()> {
        ...
        let deleted = self.db.conn.call(move |conn| {
            Ok(conn.execute("DELETE FROM gcs_buckets WHERE name=?1", rusqlite::params![n])?)
        }).await.map_err(|e| anyhow::anyhow!("{e}"))?;
        if deleted == 0 { bail!("BucketNotFound"); }
        Ok(())
    }
    ...
    pub async fn upload_object(
        ...
        Ok(GcsObject { bucket: b2, name: n2, content: Vec::new(), content_type: ct2, size, etag, created_at: now2 })
    }
    ...
    pub async fn list_objects(&self, bucket: &str, prefix: Option<&str>) -> Result<Vec<GcsObject>> {
        ...
        self.db.conn.call(move |conn| {
            ...
            let rows = stmt.query_map(rusqlite::params![b, pref], |r| Ok(GcsObject {
                bucket:       r.get(0)?,
                name:         r.get(1)?,
                content:      r.get(2)?,
    ...
    pub async fn get_object(&self, bucket: &str, name: &str) -> Result<GcsObject> {
        let b  = bucket.to_string();
        let n  = name.to_string();
        let obj: Option<GcsObject> = self.db.conn.call(move |conn| {
            Ok(conn.query_row(
                "SELECT bucket, name, content, content_type, size, etag, created_at
