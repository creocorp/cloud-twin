...
class TestSecrets:
    ...
    def test_get_secret(self, azure_http):
        azure_http.put(f"/azure/keyvault/{_VAULT}/secrets/kv-get-me", json={"value": "myval"})
    ...
    def test_list_secrets(self, azure_http):
        azure_http.put(f"/azure/keyvault/{_VAULT}/secrets/kv-list-a", json={"value": "a"})
        azure_http.put(f"/azure/keyvault/{_VAULT}/secrets/kv-list-b", json={"value": "b"})
    ...
    def test_update_secret(self, azure_http):
        azure_http.put(f"/azure/keyvault/{_VAULT}/secrets/kv-update", json={"value": "old"})
        azure_http.put(f"/azure/keyvault/{_VAULT}/secrets/kv-update", json={"value": "new"})
        r = azure_http.get(f"/azure/keyvault/{_VAULT}/secrets/kv-update")
        assert r.status_code == 200
        assert r.json()["value"] == "new"

    def test_delete_secret(self, azure_http):
        azure_http.put(f"/azure/keyvault/{_VAULT}/secrets/kv-delete-me", json={"value": "bye"})
        r = azure_http.delete(f"/azure/keyvault/{_VAULT}/secrets/kv-delete-me")
        assert r.status_code == 204

    def test_delete_then_get_returns_404(self, azure_http):
        azure_http.put(f"/azure/keyvault/{_VAULT}/secrets/kv-del-get", json={"value": "x"})
