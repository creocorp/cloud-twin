...
impl SecretsManagerService {
    ...
    pub async fn create_secret(&self, name: &str, secret_string: Option<&str>) -> Result<Secret> {
        ...
        self.get_secret(name)
            .await?
            .ok_or_else(|| anyhow::anyhow!("not found after insert"))
    }
    ...
    pub async fn get_secret_value(&self, secret_id: &str) -> Result<Secret> {
        self.get_secret(secret_id).await?.ok_or_else(|| {
            anyhow::anyhow!("ResourceNotFoundException: secret {secret_id} not found")
        })
    }
    ...
    pub async fn put_secret_value(&self, secret_id: &str, secret_string: &str) -> Result<()> {
        ...
        let affected = self
            ...
            .map_err(|e| anyhow::anyhow!("{e}"))?;
        if affected == 0 {
            bail!("ResourceNotFoundException: secret {secret_id} not found");
        }
        Ok(())
    }
