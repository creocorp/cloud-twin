...
fn svc(state: &Arc<AppState>) -> DynamoDBService {
    DynamoDBService::new(state.db.clone())
}
    ...
    match target {
        "DynamoDB_20120810.CreateTable" => {
            ...
            match svc(state).create_table(name, &ks, &ad).await {
                Ok(t) => (
                    StatusCode::OK,
                    axum::Json(
                        serde_json::json!({ "TableDescription": svc(state).table_to_json(&t) }),
                    ),
                )
                    .into_response(),
        ...
        "DynamoDB_20120810.DescribeTable" => {
            ...
            match svc(state).get_table(name).await {
                Ok(Some(t)) => (
                    StatusCode::OK,
                    axum::Json(serde_json::json!({ "Table": svc(state).table_to_json(&t) })),
                )
                    .into_response(),
        ...
        "DynamoDB_20120810.ListTables" => match svc(state).list_tables().await {
            Ok(names) => (
                StatusCode::OK,
                axum::Json(serde_json::json!({ "TableNames": names })),
            )
                .into_response(),
            Err(e) => json_error_response(
                StatusCode::INTERNAL_SERVER_ERROR,
        ...
        "DynamoDB_20120810.UpdateItem" => {
            ...
            match svc(state).update_item(name, &key, expr, ea_vals).await {
                Ok(item) => (
                    StatusCode::OK,
                    axum::Json(serde_json::json!({ "Attributes": item })),
                )
                    .into_response(),
                Err(e) if e.to_string().contains("ResourceNotFoundException") => {
                    json_error_response(
        ...
        "DynamoDB_20120810.BatchWriteItem" => {
            ...
            match svc(state).batch_write(&items).await {
                Ok(_) => (
                    StatusCode::OK,
                    axum::Json(serde_json::json!({ "UnprocessedItems": {} })),
                )
                    .into_response(),
                Err(e) => json_error_response(
                    StatusCode::INTERNAL_SERVER_ERROR,
        ...
        _ => json_error_response(
            StatusCode::BAD_REQUEST,
            "InvalidAction",
            &format!("Unknown DynamoDB target: {target}"),
        ),
    }
}
