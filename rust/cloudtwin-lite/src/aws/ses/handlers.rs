...
<VerificationToken>{}</VerificationToken>\
        ...
        "ListIdentities" => match svc(state).list_identities().await {
            Ok(ids) => {
        ...
        "GetIdentityVerificationAttributes" => {
            ...
            let mut i = 1;
            while let Some(v) = params.get(&format!("Identities.member.{i}")) {
                identities.push(v.clone());
                i += 1;
            }
            match svc(state).list_identities().await {
                Ok(all) => {
                    let map: HashMap<String, _> = all
                        .into_iter()
                        .map(|id| (id.identity.clone(), id))
                        .collect();
...
<VerificationAttributes>{entries}</VerificationAttributes>\
        ...
        "SendEmail" => {
            ...
            for prefix in &[
                ...
                let mut j = 1;
                while let Some(v) = params.get(&format!("{prefix}.member.{j}")) {
                    dests.push(v.clone());
                    j += 1;
                }
            }
        ...
        _ => xml_error_response(
            StatusCode::BAD_REQUEST,
            "InvalidAction",
            &format!("Unknown SES action: {action}"),
        ),
    }
}
...
pub fn router_v2() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route(
            "/v2/email/identities",
            post(v2_create_identity).get(v2_list_identities),
        )
        .route(
            "/v2/email/identities/:identity",
            get(v2_get_identity).delete(v2_delete_identity),
        )
}
...
async fn v2_send_email(
    State(state): State<Arc<AppState>>,
    Json(req): Json<V2SendEmailReq>,
) -> Response {
    let source = req.from.unwrap_or_default();
    let mut dests = Vec::new();
    if let Some(d) = &req.destination {
        if let Some(to) = &d.to {
            dests.extend(to.iter().cloned());
        }
        if let Some(cc) = &d.cc {
            dests.extend(cc.iter().cloned());
        }
    }
...
async fn v2_create_identity(
    State(state): State<Arc<AppState>>,
    Json(req): Json<V2CreateIdentityReq>,
) -> Response {
...
async fn v2_list_identities(State(state): State<Arc<AppState>>) -> Response {
    match svc(&state).list_identities().await {
        Ok(ids) => {
            ...
            (
                StatusCode::OK,
                Json(serde_json::json!({ "EmailIdentities": items })),
            )
                .into_response()
        }
        Err(e) => (
...
async fn v2_get_identity(
    ...
    match svc(&state).get_identity(&identity).await {
        ...
        Ok(None) => (
            ...
            Json(serde_json::json!({ "message": "Identity not found" })),
        )
            .into_response(),
        Err(e) => (
            StatusCode::INTERNAL_SERVER_ERROR,
