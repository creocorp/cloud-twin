...
class SqliteAzureStorageQueueRepository(AzureStorageQueueRepository):
    ...
    def _row(self, row) -> AzureStorageQueue:
        return AzureStorageQueue(id=row["id"], account=row["account"], name=row["name"], created_at=row["created_at"])

    async def get(self, account: str, name: str) -> Optional[AzureStorageQueue]:
        async with self._db.conn.execute(
            "SELECT * FROM azure_storage_queues WHERE account = ? AND name = ?", (account, name)
        ) as cur:
    ...
    async def delete(self, account: str, name: str) -> None:
        await self._db.conn.execute(
            "DELETE FROM azure_storage_queues WHERE account = ? AND name = ?", (account, name)
        )
        await self._db.conn.commit()
...
class SqliteAzureQueueMessageRepository(AzureQueueMessageRepository):
    ...
    def _row(self, row) -> AzureQueueMessage:
        return AzureQueueMessage(
            ...
            body=row["body"], pop_receipt=row["pop_receipt"],
            visible=bool(row["visible"]), dequeue_count=row["dequeue_count"],
    ...
    async def save(self, message: AzureQueueMessage) -> AzureQueueMessage:
        await self._db.conn.execute(
            ...
            (message.message_id, message.queue_id, message.body, message.pop_receipt,
             int(message.visible), message.dequeue_count, message.created_at or _now()),
        )
        await self._db.conn.commit()
        return message
