...
impl SqsService {
    ...
    pub async fn delete_queue(&self, queue_url: &str) -> Result<()> {
        ...
        self.db
            ...
            .call(move |conn| {
                conn.execute(
                    "DELETE FROM sqs_queues WHERE name = ?1",
    ...
    pub async fn receive_messages(&self, queue_url: &str, max: i64) -> Result<Vec<SqsMessage>> {
        let qid = self.queue_id(queue_url).await?;
        let lim = max.clamp(1, 10);
        let msgs: Vec<SqsMessage> = self
            .db
            .conn
            .call(move |conn| {
                ...
                let rows = stmt
                    .query_map(rusqlite::params![qid, lim], |r| {
                        Ok(SqsMessage {
                            message_id: r.get(0)?,
                            queue_id: r.get(1)?,
                            body: r.get(2)?,
    ...
    pub async fn get_queue_attributes(&self, queue_url: &str) -> Result<serde_json::Value> {
        ...
        let n2 = name.clone();
        let (total, invisible): (i64, i64) = self
            .db
            .conn
            .call(move |conn| {
